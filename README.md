...
### 확장자에 다른 다양한 Content-Type이 존재

### HTTP 상태코드
확장자별로 경로와 content-type를 지정하였고 리소스에 대한 처리에 따른 다양한 http 상태 코드를 보내줄 필요성을 느낌<br>
예를들면, index.html를 요창했고 보내줬으면 200, 알지 못하는 리소스라면 404 등 처리 결과에 대한 다양한 상태 코드를 보내줘야함

|상태코드|설명|
|------|---|
|1xx|요청을 받았으며 프로세스를 계속 진행|
|2xx|요청을 성공적으로 받았으며 인식했고 수용하였음|
|3xx|요청을 위해 추가 작업 조치가 필요함|
|4xx|클라이언트 오류로 요청의 문법이 잘못되어 처리할 수 없음|
|5xx|서버 오류로 요청에 대한 충족을 실패|
